import { randomUUID } from 'crypto';
import type { Feature } from '../../models/Feature';
import type { IFeatureMappingSource, MappingContext } from '../interfaces/IFeatureMappingSource';
import type { MappingCandidate } from '../models/MappingCandidate';
import type { MappingResourceType } from '../models/MappingResourceType';
import { scoreToMappingConfidenceLevel } from '../models/MappingConfidence';
import { MappingMatcher } from './MappingMatcher';

export class UIComponentMappingSource implements IFeatureMappingSource {
  public readonly sourceType: MappingResourceType = 'UI_COMPONENT';
  public readonly name = 'UIComponentMappingSource';

  public getSourceType(): MappingResourceType {
    return this.sourceType;
  }

  public supports(resourceType: MappingResourceType): boolean {
    return resourceType === 'UI_COMPONENT';
  }

  public mapFeature(feature: Feature, context: MappingContext): MappingCandidate[] {
    const candidates: MappingCandidate[] = [];
    if (!context.uiComponents) return candidates;

    for (const comp of context.uiComponents) {
      const match = MappingMatcher.matchesFeature(feature, comp.name);

      if (match.matches) {
        const candidate: MappingCandidate = {
          candidateId: `cand_ui_${randomUUID().slice(0, 8)}`,
          featureId: feature.id,
          resourceId: comp.componentId || comp.name,
          resourceType: 'UI_COMPONENT',
          proposedRole: 'UI',
          evidence: [
            {
              evidenceId: `ev_ui_${randomUUID().slice(0, 8)}`,
              sourceType: 'UI_COMPONENT',
              sourceId: comp.componentId || comp.name,
              evidenceType: 'UI_PRESENTATION_LAYER',
              description: `UI Component "${comp.name}" renders interface for "${feature.name}" in ${comp.filePath}`,
              strength: 0.8,
              confidence: match.confidence,
              metadata: {
                name: comp.name,
                filePath: comp.filePath,
                props: comp.props,
              },
              timestamp: Date.now(),
            },
          ],
          score: match.confidence,
          confidence: {
            level: scoreToMappingConfidenceLevel(match.confidence),
            score: match.confidence,
            reasons: [match.reason],
          },
          sources: ['UI_COMPONENT'],
          conflicts: [],
          status: 'DETECTED',
          createdAt: Date.now(),
          updatedAt: Date.now(),
        };
        candidates.push(candidate);
      }
    }

    return candidates;
  }

  public discoverMappings(context: MappingContext): MappingCandidate[] {
    return [];
  }
}
